-- Identity fields for dueño and empleado on usuarios_negocio.
...
-- ---------------------------------------------------------------------------
-- registrar_empleado: require identity
-- ---------------------------------------------------------------------------

drop function if exists public.registrar_empleado(text);
...
grant execute on function public.registrar_empleado(
  text, text, text, text, text
) to authenticated;

-- ---------------------------------------------------------------------------
-- registrar_negocio: store dueño identity (keeps afiliado + ubicación params)
-- ---------------------------------------------------------------------------

drop function if exists public.registrar_negocio(
  text, text, text, text, double precision, double precision
);

drop function if exists public.registrar_negocio(
  text, text, text, text, double precision, double precision, uuid
);
...
-- ---------------------------------------------------------------------------
-- listar empleados with identity + email
-- ---------------------------------------------------------------------------

drop function if exists public.listar_empleados_negocio(uuid);
...
begin
  ...
  where un.negocio_id = p_negocio_id
    and un.rol = 'empleado'
  order by un.nombre nulls last, u.email nulls last, un.id;
end;
$$;
