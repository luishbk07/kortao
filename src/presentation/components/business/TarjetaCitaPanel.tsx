...
export const TarjetaCitaPanel = ({
  ...
  const marcandoAtendida = ocupada && accionEnCurso?.tipo === 'atendida'
      ...
      {puedeCancelar || puedeMarcarAtendida ? (
        <CardActions sx={{ px: 2, pb: 2, flexWrap: 'wrap', gap: 1 }}>
          {puedeMarcarAtendida ? (
            ...
            >
              {marcandoAtendida ? 'Marcando...' : 'Marcar como atendida'}
            </Button>
          ) : null}
          {puedeCancelar ? (
            <Button
